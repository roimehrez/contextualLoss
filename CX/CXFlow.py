...
from CX.NNWFlow import NNWFlow
...
class CXFlow:
    ...
    def CX_loss(T_features, I_features, distance=Distance.L2, nnsigma=float(1.0)):
        ...
        with tf.name_scope('CX'):
            nnw_flow = NNWFlow.create(I_features, T_features, distance, nnsigma)
            # sum_normalize:
    ...
    def __init__(self, sT, nnw_flow: NNWFlow):
        if nnw_flow is not None:
            self.nnw_flow = nnw_flow
            self.nnw_NHWC = nnw_flow.nnw_NHWC
        self.sT = sT
